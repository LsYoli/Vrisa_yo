...
export const useUsers = () => {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    const [userRes, roleRes, permissionRes, rolePermRes] = await Promise.all([
      supabase.from('usuario').select('*').order('id', { ascending: true }),
      supabase.from('rol').select('*').order('id', { ascending: true }),
      supabase.from('permiso').select('*').order('id', { ascending: true }),
  ...
  const updateUserRole = async (userId, roleId) => {
    setLoading(true);
    const { error: updateError } = await supabase.from('usuario').update({ rol_id: roleId }).eq('id', userId);
    if (updateError) {
